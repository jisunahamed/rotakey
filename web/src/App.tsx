...
function ProvidersPage({ notify }: { notify: (message: string, tone?: "success" | "danger") => void }) {
  ...
  return (
    <div className="resource-page provider-page">
      ...
      {/* The same sheet Models opens, with the provider fixed: adding a route while
...
/** The limits on a key, short enough to sit at the end of its row.
 *
 *  It used to build the summary itself, from `Object.entries`, which put the seven
 *  dimensions in whatever order the object happened to carry them and printed the
 *  raw figures without separators — "TPM 400000". Both come from `lib/copy` now,
 *  in the console's one order, and the full text with every acronym spelled out
 *  goes on the title, because "RPM" is three letters read aloud. */
function LimitSummary({ policy }: { policy: RatePolicy }) {
  const summary = rateSummary(policy);
  if (summary === "") return <span className="muted">No limits</span>;
  return <span className="mono-summary" title={rateSummaryFull(policy)}>{summary}</span>;
}
